...
#include <assert.h>
#include "moThresholdColoredModule.h"
#include "../moLog.h"
#include "cv.h"

MODULE_DECLARE(ThresholdColored, "native", "Thresholding to throw away all values below or above certain threshold");
...
moThresholdColoredModule::~moThresholdColoredModule() 
{
}

void moThresholdColoredModule::stop() {
...
int moThresholdColoredModule::getCvType(const std::string &filter) 
{
	if ( filter == "binary" )
...
int moThresholdColoredModule::getCvAdaptativeType(const std::string &filter) 
{
	if ( filter == "binary" )
		return CV_THRESH_BINARY;
	if ( filter == "binary_inv" )
...
int moThresholdColoredModule::getCvMode(const std::string &filter) 
{
	if ( filter == "mean" )
...
void moThresholdColoredModule::applyFilter(IplImage *)
{
